...
def get_platform(platform_name: str = "fastest") -> Platform:
    """Get the specified platform or fall back to alternatives if not available."""
    # Get all available platforms
    available_platforms = {Platform.getPlatform(i).getName(): Platform.getPlatform(i) 
                         for i in range(Platform.getNumPlatforms())}
    
    logger.info(f"Available platforms: {', '.join(available_platforms.keys())}")
    ...
    if platform_name == "fastest":
        ...
        logger.info(f"Selected fastest platform: {platform.getName()}")
        return platform
    ...
    # Fallback logic if requested platform isn't available
    preferred_order = ["OpenCL", "CPU", "CUDA", "Reference"]
    for p in preferred_order:
        if p in available_platforms:
            logger.warning(f"Requested platform {platform_name} not available. Using {p} instead.")
            return available_platforms[p]
    
    # If we get here, just use whatever's available
    logger.warning(f"Requested platform {platform_name} not available. Using default.")
    return Platform.getPlatform(0)


def get_fastest_platform() -> Platform:
    return get_platform("fastest")


def get_platform_properties(platform_name: str, custom_properties: dict = None) -> dict:
    """Generate appropriate default properties for the given platform, with custom overrides."""
    properties = {}
    
    # Start with platform-specific defaults
    if platform_name.upper() == "OpenCL":
        # OpenCL default properties
        properties = {
            "DeviceIndex": "0",        # Use first device by default
            "Precision": "mixed",      # mixed precision is usually a good balance
            "UseCpuPme": "false",      # Use GPU for PME calculations 
        }
    elif platform_name.upper() == "CUDA":
        # CUDA default properties 
        properties = {
            "DeviceIndex": "0",
            "Precision": "mixed",
            "UseCpuPme": "false",
...
def setup_simulation(modeller: Modeller, system: System, platform: Platform, properties: dict = None) -> Simulation:
...
def optimize_ligand_in_pocket(
    ...
    try:
        ...
        # If conformer generation fails, try to use the existing conformers.
    ...
    # Force OpenCL if "fastest" was requested (bypassing CUDA)
    if platform_name.lower() == "fastest":
        logger.info("'fastest' platform requested, but forcing OpenCL to avoid CUDA issues")
    ...
    # Try to get the requested platform or fall back to CPU
    if platform_name in available_platforms:
        platform = available_platforms[platform_name]
    else:
        # Fallback order - explicitly avoid CUDA
        for p in ["OpenCL", "CPU", "Reference"]:
            if p in available_platforms:
                platform = available_platforms[p]
                logger.warning(f"Requested platform {platform_name} not available. Using {p} instead.")
                platform_name = p  # Update name for property selection
                break
        else:
            raise RuntimeError(f"No suitable platform found among: {', '.join(available_platforms.keys())}")
    
    # Get properties for the selected platform
    properties = get_platform_properties(platform_name, platform_properties)
    logger.info(f"Using platform: {platform.getName()} with properties: {properties}")
    
    # Setup simulation with platform properties
    try:
        simulation = setup_simulation(modeller, system, platform, properties)
        
        # Log platform information
        actual_platform = simulation.context.getPlatform().getName()
        logger.info(f"Simulation running on platform: {actual_platform}")
        
    except Exception as e:
        logger.error(f"Error setting up simulation with {platform_name}: {e}")
        # Try one more fallback to CPU if we haven't already
        if platform_name != "CPU" and "CPU" in available_platforms:
            logger.info("Falling back to CPU platform")
            platform = available_platforms["CPU"]
            properties = get_platform_properties("CPU", None)
            simulation = setup_simulation(modeller, system, platform, properties)
        else:
            raise RuntimeError(f"No suitable platform found among: {', '.join(available_platforms.keys())}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--platform_props", type=str, default=None,
                       help="Optional platform properties as comma-separated key:value pairs (e.g., 'DeviceIndex:0,Precision:mixed')")
    ...
    # Parse platform properties if provided
    platform_properties = None
    if args.platform_props:
        platform_properties = {}
        props_list = args.platform_props.split(',')
        for prop in props_list:
            if ':' in prop:
                key, value = prop.split(':')
                platform_properties[key.strip()] = value.strip()
